...
default['cookbook']['harden']['controls']['aide'].tap do |control|
  ...
  control['sha512'].tap do |configuration|
    configuration['managed'] = true
    configuration['title'] = 'SV-258136: RHEL 9 must use a file integrity tool that is configured to use FIPS 140-3-approved cryptographic hashes for validating file contents and directories.'
    configuration['values'] = [
  ...
  control['cryptographic'].tap do |configuration|
    configuration['managed'] = true
    configuration['title'] = 'SV-258137: RHEL 9 must use cryptographic mechanisms to protect the integrity of audit tools.'
    configuration['values'] = [
  ...
  control['attrs'].tap do |configuration|
    configuration['managed'] = true
    configuration['title'] = 'SV-258139: RHEL 9 must be configured so that the file integrity tool verifies extended attributes.'
    configuration['path'] = '/etc/aide.conf'
    configuration['values'] = [
      'All=p+i+n+u+g+s+m+S+sha512+acl+xattrs+selinux',
      ]
