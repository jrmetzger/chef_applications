...
default['cookbook']['harden']['controls']['audit'].tap do |control|
  ...
  control['space_left'].tap do |configuration|
    configuration['managed'] = true
    configuration['title'] =
      'SV-258155: RHEL 9 must allocate audit record storage capacity to store at least one weeks worth of audit records.'
  ...
  control['setsebool'].tap do |configuration|
    ...
    configuration['rule'] =
      '-a always,exit -F path=/usr/sbin/setsebool -F perm=x -F auid>=1000 -F auid!=unset -F key=privileged-unix-update'
  end
  ...
  control['passwd_command'].tap do |configuration|
    configuration['managed'] = true
